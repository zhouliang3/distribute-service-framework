 ...
 */
package com.ctg.itrdc.janus.registry.integration;
...
import com.ctg.itrdc.janus.rpc.cluster.*;
import com.ctg.itrdc.janus.rpc.cluster.directory.AbstractDirectory;
...
import com.ctg.itrdc.janus.rpc.protocol.InvokerWrapper;
...
public class RegistryDirectory<T> extends AbstractDirectory<T> implements NotifyListener {
    ...
    private URL mergeUrl(URL providerUrl){
        ...
        if ((providerUrl.getPath() == null || providerUrl.getPath().length() == 0)
                && "janus".equals(providerUrl.getProtocol())) { // 兼容1.0
            //fix by tony.chenl JANUS-44
    ...
    public List<Invoker<T>> doList(Invocation invocation) {
        if (forbidden) {
            throw new RpcException(RpcException.FORBIDDEN_EXCEPTION, "Forbid consumer " +  NetUtils.getLocalHost() + " access service " + getInterface().getName() + " from registry " + getUrl().getAddress() + " use janus version " + Version.getVersion() + ", Please check registry access list (whitelist/blacklist).");
        }
