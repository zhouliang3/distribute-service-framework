...
public class EnumBak {
    ...
    public void testNormal(){
        ...
        URL serviceurl = URL.valueOf("janus://127.0.0.1:"+port+"/test?proxy=jdk"
...
//        invoker.destroy();
    ...
    public void testExportService() throws InterruptedException{
        int port = NetUtils.getAvailablePort();
        URL serviceurl = URL.valueOf("janus://127.0.0.1:"+port+"/test?proxy=jdk&timeout="+Integer.MAX_VALUE
                );
        DemoService demo = new DemoServiceImpl();
        Invoker<DemoService> invoker = proxy.getInvoker(demo, DemoService.class, serviceurl);
        protocol.export(invoker);
...
//        reference.destroy();
    ...
    public void testNormalEnum(){
        int port = NetUtils.getAvailablePort();
        URL serviceurl = URL.valueOf("janus://127.0.0.1:"+port+"/test?timeout="+Integer.MAX_VALUE
                );
        DemoService demo = new DemoServiceImpl();
        Invoker<DemoService> invoker = proxy.getInvoker(demo, DemoService.class, serviceurl);
        protocol.export(invoker);
...
//    @Test
    ...
    public void testEnumCompat(){
        int port = 20880;
        URL consumerurl = URL.valueOf("janus://127.0.0.1:"+port+"/test?timeout="+Integer.MAX_VALUE
        );
        Invoker<DemoService> reference = protocol.refer(DemoService.class, consumerurl);
        DemoService demoProxy = (DemoService)proxy.getProxy(reference);
        Type type = demoProxy.enumlength(Type.High);
...
//    @Test
    ...
    public void testGenricCustomArg(){
        ...
        int port = 20880;
        URL consumerurl = URL.valueOf("janus://127.0.0.1:"+port+"/test?timeout=2000000"
        );
        Invoker<GenericService> reference = protocol.refer(GenericService.class, consumerurl);
        ...
        Object obj = demoProxy.$invoke("get", new String[]{"com.ctg.itrdc.janus.rpc.CustomArgument"}, new Object[]{arg});
        System.out.println("obj---------->"+obj);
        reference.destroy();
    }
