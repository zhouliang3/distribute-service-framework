...
import com.ctg.itrdc.janus.common.serialize.support.SerializableClassRegistry;
import com.ctg.itrdc.janus.common.serialize.support.SerializationOptimizer;
...
import com.ctg.itrdc.janus.rpc.protocol.AbstractProtocol;

/**
 * janus protocol support.
 *
 * @author qian.lei
 * @author william.liangf
 * @author chao.liuc
 */
public class JanusProtocol extends AbstractProtocol {

    public static final String NAME = "janus";

    public static final String COMPATIBLE_CODEC_NAME = "janus1compatible";
    ...
    private static JanusProtocol INSTANCE;

    public JanusProtocol() {
        INSTANCE = this;
    }
    
    public static JanusProtocol getJanusProtocol() {
        if (INSTANCE == null) {
            ExtensionLoader.getExtensionLoader(Protocol.class).getExtension(JanusProtocol.NAME); // load
        }
        return INSTANCE;
    }
    ...
    Invoker<?> getInvoker(Channel channel, Invocation inv) throws RemotingException{
        ...
        JanusExporter<?> exporter = (JanusExporter<?>) exporterMap.get(serviceKey);
    ...
    public <T> Exporter<T> export(Invoker<T> invoker) throws RpcException {
        ...
        JanusExporter<T> exporter = new JanusExporter<T>(invoker, key, exporterMap);
    ...
    private ExchangeServer createServer(URL url) {
        ...
        url = url.addParameter(Constants.CODEC_KEY, Version.isCompatibleVersion() ? COMPATIBLE_CODEC_NAME : JanusCodec.NAME);
    ...
    public <T> Invoker<T> refer(Class<T> serviceType, URL url) throws RpcException {
        ...
        // create rpc invoker.
        JanusInvoker<T> invoker = new JanusInvoker<T>(serviceType, url, getClients(url), invokers);
...
//                logger.warn(new IllegalStateException("client is closed,but stay in clientmap .client :"+ client));
    ...
    private ExchangeClient initClient(URL url) {
        ...
        String version = url.getParameter(Constants.JANUS_VERSION_KEY);
        boolean compatible = (version != null && version.startsWith("1.0."));
        url = url.addParameter(Constants.CODEC_KEY, Version.isCompatibleVersion() && compatible ? COMPATIBLE_CODEC_NAME : JanusCodec.NAME);
        //默认开启heartbeat
        url = url.addParameterIfAbsent(Constants.HEARTBEAT_KEY, String.valueOf(Constants.DEFAULT_HEARTBEAT));
    ...
    public void destroy() {
        for (String key : new ArrayList<String>(serverMap.keySet())) {
            ...
            if (server != null) {
                try {
                    if (logger.isInfoEnabled()) {
                        logger.info("Close janus server: " + server.getLocalAddress());
                    }
        ...
        for (String key : new ArrayList<String>(referenceClientMap.keySet())) {
            ...
            if (client != null) {
                try {
                    if (logger.isInfoEnabled()) {
                        logger.info("Close janus connect: " + client.getLocalAddress() + "-->" + client.getRemoteAddress());
                    }
                    client.close();
                } catch (Throwable t) {
                    logger.warn(t.getMessage(), t);
