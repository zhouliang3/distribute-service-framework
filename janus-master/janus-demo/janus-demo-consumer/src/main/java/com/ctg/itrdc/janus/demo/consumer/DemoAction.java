 ...
 */
package com.ctg.itrdc.janus.demo.consumer;

import com.ctg.itrdc.janus.demo.bid.*;
import com.ctg.itrdc.janus.demo.user.User;
import com.ctg.itrdc.janus.demo.user.facade.AnotherUserRestService;
...
//        System.out.println(">>>>> Total time consumed:" + (System.currentTimeMillis() - start));
        ...
        //inote 暂时注释掉   分开测试-----  System.out.println("SUCCESS: registered user with id " + anotherUserRestService.registerUser(user).getId());

        RpcContext.getContext().setAttachment("clientName", "demo");
        RpcContext.getContext().setAttachment("clientImpl", "janus");
        //inote 暂时注释掉   分开测试-----System.out.println("SUCCESS: got user " + anotherUserRestService.getUser(1L));
    }
