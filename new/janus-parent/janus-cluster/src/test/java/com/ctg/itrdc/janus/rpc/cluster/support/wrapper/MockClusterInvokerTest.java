...
import com.ctg.itrdc.janus.rpc.cluster.support.AbstractClusterInvoker;
import com.ctg.itrdc.janus.rpc.support.MockProtocol;
...
/**
 * iundo 此测试用例会抛出很多异常，但是，并不影响测试结果，依然测试通过
 */
public class MockClusterInvokerTest {
	...
	public void testMockInvokerFromOverride_Invoke_force_throwCustemException() throws Throwable{
		URL url = URL.valueOf("remote://1.2.3.4/"+IHelloService.class.getName())
				.addParameter("getBoolean2.mock","force:throw com.ctg.itrdc.janus.rpc.cluster.support.wrapper.MyMockException")
				.addParameter("invoke_return_error", "true" );
		Invoker<IHelloService> cluster = getClusterInvoker(url);        
		//方法配置了mock
        RpcInvocation invocation = new RpcInvocation();
