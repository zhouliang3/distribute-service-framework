...
public class ConfigUtils {
	 ...
	 */
	public static List<String> mergeValues(Class<?> type, String cfg,
			List<String> def) {
	...
	public static Properties getProperties() {
		if (PROPERTIES == null) {
			synchronized (ConfigUtils.class) {
				if (PROPERTIES == null) {
					String path = System
							.getProperty(Constants.JANUS_PROPERTIES_KEY);
					if (path == null || path.length() == 0) {
						path = System.getenv(Constants.JANUS_PROPERTIES_KEY);
						if (path == null || path.length() == 0) {
							path = Constants.DEFAULT_JANUS_PROPERTIES;
						}
					}
	 ...
	 * 根据配置项key，获取配置值
	 * 
	 * 先从系统属性获取，获取不到从janus.properties配置取
	 * 
	 * @param key
	 * @return
	 */
	...
	public static String getProperty(String key, String defaultValue) {
		...
		return replaceProperty(properties.getProperty(key, defaultValue),
				(Map) properties);
	}
	...
	public static Properties loadProperties(String fileName,
		...
		if (fileName.startsWith("/")) {
			return loadPropertiesFromLocalFile(fileName, properties);
		}

		List<java.net.URL> list = getResourceUrls(fileName);
		...
		if (!allowMultiFile) {
			if (list.size() > 1) {
				String errMsg = String
						.format("only 1 %s file is expected, but %d janus.properties files found on class path: %s",
								fileName, list.size(), list.toString());
				logger.warn(errMsg);
				// throw new IllegalStateException(errMsg); // see
				// http://code.alibabatech.com/jira/browse/JANUS-133
			}
	 ...
	 * @return
	 */
	private static List<java.net.URL> getResourceUrls(String fileName) {
	 ...
	 * @return
	 */
	private static Properties loadPropertiesFromLocalFile(String fileName,
