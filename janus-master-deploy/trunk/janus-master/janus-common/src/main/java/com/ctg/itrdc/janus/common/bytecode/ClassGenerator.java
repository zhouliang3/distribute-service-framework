...
public final class ClassGenerator {
	...
	/**
	 * 创建一个ClassGenerator实例
	 * 
	 * 采用当前系统的Context Class loader创建classPool实例
	 * 
	 * @return
	 */
	public static ClassGenerator newInstance() {
		return new ClassGenerator(getClassPool(Thread.currentThread()
				.getContextClassLoader()));
	}

	/**
	 * 根据ClassLoader对象，创建ClassGenerator实例
	 * 
	 * @param loader
	 * @return
	 */
	...
	/**
	 * 根据ClassLoader对象，获取ClassPool对象<br/>
	 * 如果loader为null，返回系统默认的Default ClassPool <br/>
	 * 否则从缓存中获取，获取到直接返回。否则创建一个新的ClassPool
	 * 
	 * @param loader
	 * @return
	 */
	...
	public ClassGenerator addField(String name, int mod, Class<?> type,
		...
		sb.append(modifier(mod)).append(' ').append(ReflectUtils.getName(type))
				.append(' ');
		sb.append(name);
	...
	public ClassGenerator addMethod(String name, int mod, Class<?> rt,
		...
		StringBuilder sb = new StringBuilder();
		sb.append(modifier(mod)).append(' ').append(ReflectUtils.getName(rt))
				.append(' ').append(name);
		sb.append('(');
	...
	public ClassGenerator addConstructor(int mod, Class<?>[] pts,
	...
	public Class<?> toClass() {
		return toClass(getClass().getClassLoader(), getClass()
				.getProtectionDomain());
	}
		...
		try {
			...
			if (mClassName == null)
				mClassName = (mSuperClass == null
						|| javassist.Modifier.isPublic(ctcs.getModifiers()) ? ClassGenerator.class
				...
				for (String code : mConstructors) {
					if (code.charAt(0) == ':') {
						mCtc.addConstructor(CtNewConstructor.copy(
								getCtConstructor(mCopyConstructors.get(code
										.substring(1))), mCtc, null));
					} else {
						String[] sn = mCtc.getSimpleName().split("\\$+"); // inner
																			...
																			// include
																			// $.
						mCtc.addConstructor(CtNewConstructor.make(code
								.replaceFirst(SIMPLE_NAME_TAG,
										sn[sn.length - 1]), mCtc));
					}
				}
			}
	...
	private CtConstructor getCtConstructor(Constructor<?> c)
			throws NotFoundException {
		return getCtClass(c.getDeclaringClass()).getConstructor(
