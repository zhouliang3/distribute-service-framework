...
import com.ctg.itrdc.janus.config.annotation.Reference;
...
import com.ctg.itrdc.janus.rpc.StaticContext;
import com.ctg.itrdc.janus.rpc.cluster.Cluster;
import com.ctg.itrdc.janus.rpc.cluster.directory.StaticDirectory;
import com.ctg.itrdc.janus.rpc.cluster.support.AvailableCluster;
import com.ctg.itrdc.janus.rpc.cluster.support.ClusterUtils;
import com.ctg.itrdc.janus.rpc.protocol.injvm.InjvmProtocol;
	...
	private void init() {
		...
		if (interfaceName == null || interfaceName.length() == 0) {
			throw new IllegalStateException(
					"<janus:reference interface=\"\" /> interface not allow null!");
		}
		// 获取消费者全局配置
		checkDefault();
		...
		if (resolve == null || resolve.length() == 0) {
			resolveFile = System.getProperty("janus.resolve.file");
			if (resolveFile == null || resolveFile.length() == 0) {
				File userResolveFile = new File(new File(
						System.getProperty("user.home")),
						"janus-resolve.properties");
		...
		if (resolve != null && resolve.length() > 0) {
			...
			if (logger.isWarnEnabled()) {
				if (resolveFile != null && resolveFile.length() > 0) {
					logger.warn("Using default janus resolve file "
				...
				if (urls == null || urls.size() == 0) {
					throw new IllegalStateException(
							"No such any registry to reference "
									...
									+ " use janus version "
									+ Version.getVersion()
									+ ", please config <janus.registry.address=\"...\" /> to your spring config.");
				}
			}
		...
		if (c && !invoker.isAvailable()) {
			throw new IllegalStateException(
					"Failed to check the status of the service "
							...
							+ " use janus version " + Version.getVersion());
		}
		if (logger.isInfoEnabled()) {
			logger.info("Refer janus service " + interfaceClass.getName()
