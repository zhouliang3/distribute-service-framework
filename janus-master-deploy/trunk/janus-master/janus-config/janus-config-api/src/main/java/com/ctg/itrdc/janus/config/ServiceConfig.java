...
import com.ctg.itrdc.janus.config.annotation.JanusService;
...
import com.ctg.itrdc.janus.rpc.ServiceClassHolder;
import com.ctg.itrdc.janus.rpc.cluster.ConfiguratorFactory;
...
public class ServiceConfig<T> extends AbstractServiceConfig {
	...
	public ServiceConfig(JanusService service) {
		appendAnnotation(JanusService.class, service);
	}
	...
	protected synchronized void doExport() {
		...
		if (interfaceName == null || interfaceName.length() == 0) {
			throw new IllegalStateException(
					"<janus:service interface=\"\" /> interface not allow null!");
		}
		checkDefault();
		if (provider != null) {
			if (application == null) {
	...
	private void doExportUrls() {
		...
		for (ProtocolConfig protocolConfig : protocols) {
			doExportUrlsForSingleProtocol(protocolConfig, registryURLs);
		}
	}

	private void doExportUrlsForSingleProtocol(ProtocolConfig protocolConfig,
		...
		map.put(Constants.JANUS_VERSION_KEY, Version.getVersion());
				...
				if (arguments != null && arguments.size() > 0) {
					for (ArgumentConfig argument : arguments) {
						...
						if (argument.getType() != null
							...
							throw new IllegalArgumentException(
									"argument config must set index or type attribute.eg: <janus:argument index='0' .../> or <janus:argument type=xxx .../>");
						}
		...
		if (!Constants.SCOPE_NONE.toString().equalsIgnoreCase(scope)) {
			...
			if (!Constants.SCOPE_LOCAL.toString().equalsIgnoreCase(scope)) {
				if (logger.isInfoEnabled()) {
					logger.info("Export janus service "
				...
				if (registryURLs != null && registryURLs.size() > 0
					...
					for (URL registryURL : registryURLs) {
						...
						if (logger.isInfoEnabled()) {
							logger.info("Register janus service "
	...
	private void exportLocal(URL url) {
		if (!Constants.LOCAL_PROTOCOL.equalsIgnoreCase(url.getProtocol())) {
			...
			// modified by lishen
			ServiceClassHolder.getInstance().pushServiceClass(
					getServiceClass(ref));

			Exporter<?> exporter = protocol.export(proxyFactory.getInvoker(ref,
					(Class) interfaceClass, local));
			exporters.add(exporter);
			logger.info("Export janus service " + interfaceClass.getName()
					+ " to local registry");
		}
	}
	...
	@Deprecated
	private static final List<ProtocolConfig> convertProviderToProtocol(
	...
	@Deprecated
	private static final List<ProviderConfig> convertProtocolToProvider(
	...
	@Deprecated
	private static final ProtocolConfig convertProviderToProtocol(
	...
	@Deprecated
	private static final ProviderConfig convertProtocolToProvider(
