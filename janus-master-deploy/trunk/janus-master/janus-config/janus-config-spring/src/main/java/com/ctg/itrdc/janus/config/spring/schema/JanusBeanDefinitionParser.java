...
import com.ctg.itrdc.janus.config.ArgumentConfig;
 ...
 * @export
 */
public class JanusBeanDefinitionParser implements BeanDefinitionParser {

    private static final Logger logger = LoggerFactory.getLogger(JanusBeanDefinitionParser.class);

    private final Class<?> beanClass;

    private final boolean required;

    public JanusBeanDefinitionParser(Class<?> beanClass, boolean required) {
    ...
    private static BeanDefinition parse(Element element, ParserContext parserContext, Class<?> beanClass, boolean required) {
                    	...
                    	if (value.length() > 0) {
                    		if ("registry".equals(property) && RegistryConfig.NO_AVAILABLE.equalsIgnoreCase(value)) {
                                ...
                                if (isPrimitive(type)) {
                                    ...
                                    if ("janus:provider".equals(element.getTagName())) {
                                        logger.warn("Recommended replace <janus:provider protocol=\"" + value + "\" ... /> to <janus:protocol name=\"" + value + "\" ... />");
                                    }
                                    // 兼容旧版本配置
        ...
        if(RegistryConfig.class.equals(beanClass)){//解析dsn属性，根据dsn属性配置，获取环境变量
            PropertyValue dsn1 = beanDefinition.getPropertyValues().getPropertyValue("dsn");
            if(dsn1!=null) {
                String dsn = dsn1.getValue().toString();
                String address = System.getenv(dsn);
                ...
                if (node instanceof Element) {
                    ...
                    if ("method".equals(node.getNodeName()) || "method".equals(node.getLocalName())) {
                        ...
                        if (methodName == null || methodName.length() == 0) {
                            throw new IllegalStateException("<janus:method> name attribute == null");
                        }
                        if (methods == null) {
