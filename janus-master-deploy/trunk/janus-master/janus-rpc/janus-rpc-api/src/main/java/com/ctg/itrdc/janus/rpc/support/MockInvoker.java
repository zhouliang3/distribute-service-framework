...
final public class MockInvoker<T> implements Invoker<T> {
	...
	/**
	 * 执行mock调用处理<br/>
	 * 1.如果mock参数值为空，抛RpcException异常<br/>
	 * 2.如果mock参数值等于return，直接返回null <br/>
	 * 3.如果mock参数值为return:xxxx，直接返回其中的xxxx <br/>
	 * 4.如果mock参数值以throw开头， 抛出该异常<br/>
	 * 5.如果mock不为空，并且为其它值，表示本地的xxxxServiceMock，进行本地调用。
	 */
	public Result invoke(Invocation invocation) throws RpcException {
		String mock = getUrl().getParameter(
		...
		if (StringUtils.isBlank(mock)) {
			throw new RpcException(new IllegalAccessException(
					"mock can not be null. url :" + url));
		}
		...
		if (mock.startsWith(Constants.RETURN_PREFIX)) {
			...
			try {
				...
				throw new RpcException("mock return invoke error. method :"
						+ invocation.getMethodName() + ", mock:" + mock
						+ ", url: " + url, ew);
			}
		}
		...
		try {
			...
			throw new RpcException("Failed to create mock implemention class "
					+ mock, t);
		}
	...
	private Throwable getThrowable(String throwstr) {
		...
		if (throwable != null) {
			...
			try {
				...
				constructor = ReflectUtils.findConstructor(bizException,
						String.class);
				t = (Throwable) constructor
						.newInstance(new Object[] { " mocked exception for Service degradation. " });
				if (throwables.size() < 1000) {
					throwables.put(throwstr, t);
				}
			} catch (Exception e) {
				throw new RpcException("mock throw error :" + throwstr
						+ " argument error.", e);
			}
			return t;
		}
	...
	/**
	 * 根据mockService名称，获取具体的服务Invoker对象
	 * 
	 * 本地的Mock对象
	 * 
	 * 
	...
	private Invoker<T> getInvoker(String mockService) {
		...
		if (invoker != null) {
			...
			Class<T> serviceType = (Class<T>) ReflectUtils.forName(url
					.getServiceInterface());
			...
			if (!serviceType.isAssignableFrom(mockClass)) {
				throw new IllegalArgumentException(
						"The mock implemention class " + mockClass.getName()
	...
	private String normallizeMock(String mock) {
		if (mock == null || mock.trim().length() == 0) {
			return mock;
		} else if (ConfigUtils.isDefault(mock)
				|| "fail".equalsIgnoreCase(mock.trim())
	...
	public static Object parseMockValue(String mock, Type[] returnTypes)
		...
		if ("empty".equals(mock)) {
			value = ReflectUtils.getEmptyObject(returnTypes != null
					&& returnTypes.length > 0 ? (Class<?>) returnTypes[0]
					: null);
			...
			value = false;
				...
				&& (mock.startsWith("\"") && mock.endsWith("\"") || mock
						.startsWith("\'") && mock.endsWith("\'"))) {
			value = mock.subSequence(1, mock.length() - 1);
		} else if (returnTypes != null && returnTypes.length > 0
				&& returnTypes[0] == String.class) {
			value = mock;
